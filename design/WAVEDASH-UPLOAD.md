...
**Currency: B39 shipped & aligned (git `main == origin == 79a7fe5`; Wavedash playtest build `mn74s1g4rpv6qss2jqmmf7nr6h8e8exv`).** Build **12,775 B / 537 free (96.0%)**. Copy re-verified against `src/*.js`. The world is a **480×38 spine-and-loop** map — a surface spine with the spawn hub at center (tile 240), difficulty ramping outward, distinct zone shapes, a parallel sky highway (traverse at altitude or on the ground), and an underground **cave network** carved into rock (three distinct systems with chambers, skinny tunnels, single-entrance secrets, 6 chests, cave voids you platform across to escape). Platform placement is enforced by a stable-math geometry gate (tools/map-geometry.mjs). B38 fixed iPhone landscape/fullscreen (canvas sized from its own box, not visualViewport). The **skill tree is gone** (abilities always-on); all six enemies are **grounded walkers**.

...
**State snapshot (2026-09-12):** B38 is **shipped & aligned** — git `main == origin == 95520c1`, Wavedash playtest build **mn795432nrhw7c72zbvy3nhapn8e8p5h**, built at **12,648 / 13,312 B (664 B free, 95.0%)**, save **v44**, single slot `uni_s0`. The world is a **480×34 SPINE-AND-LOOP** map: a readable surface SPINE with the spawn HUB at center (tile 240), difficulty ramping OUTWARD to the far edges, distinct zone SHAPES (PEAK vertical ladder · CANOPY climb · MEADOW rest+alcoves · EASTRUN pit-gauntlet · SUMMIT ascending staircase), and **discrete cave SPOKES** (VIOLET shallow / CENTRAL secret / INDIGO deep — each a vertical dungeon that loops back to the surface; underground is otherwise sealed rock, NOT a highway). Platforms are deliberately spaced (aligned alcoves, even-gap arcs, monotonic staircases). B38 also fixes iPhone landscape/fullscreen: the canvas is sized from its own laid-out box (position:fixed;inset:0), not visualViewport, curing the "fits half the screen" bug. Since B28: same feature set; all six enemies are **grounded walkers**; the skill tree is **gone** (abilities always-on). For the authoritative running state see the **Definitive State** knowledge entry.

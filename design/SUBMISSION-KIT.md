...
**State snapshot (2026-09-12):** B36 is **shipped & aligned** — git `main == origin == 4a76691`, Wavedash playtest build **mn7avv6bvvzh7hc7r73dda8mz58e9pm4**, built at **12,643 / 13,312 B (669 B free, 95.0%)**, save **v44**, single slot `uni_s0`. The world is a compact **480×30** 3-band map (SKY / uniform GROUND band / CAVES): **shallow single-level caves** (floor row 28), a **full-width developed underground** (39%→99% built), spawn **centered at tile 240**, and dense sky+cave platforming with chests/enemies/bosses spread evenly. Since B28: same feature set, byte-positive; all six enemies are **grounded walkers**; the skill tree is **gone** (abilities always-on). For the authoritative running state see the **Definitive State** knowledge entry.

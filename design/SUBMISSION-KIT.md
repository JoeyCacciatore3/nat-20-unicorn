...
**State snapshot (2026-09-12):** B35 is **shipped & aligned** — git `main == origin == 366329d`, last built at **12,454 / 13,312 B (858 B free, 93.6%)**, save **v44**, single slot `uni_s0`. **B36 map-rework sits in the working tree (uncommitted, audit-green, not yet rebuilt):** the world was compacted further to **480×30** (shallow single-level caves, floor row 28) and the spawn/paddock was **centered at tile 240**. Big changes since B28: the world was re-authored into a compact **3-band** map (SKY / uniform GROUND band / CAVES) — same feature set, byte-positive; all six enemies are now **grounded walkers** (the floaters were flipped upright); the skill tree is **gone** (abilities always-on). For the authoritative running state see the **Definitive State** knowledge entry.

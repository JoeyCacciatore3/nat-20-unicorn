...
**State snapshot (2026-09-12):** B39 is **shipped & aligned** — git `main == origin == 79a7fe5`, Wavedash playtest build **mn74s1g4rpv6qss2jqmmf7nr6h8e8exv**, built at **12,775 / 13,312 B (537 B free, 96.0%)**, save **v44**, single slot `uni_s0`. The world is a **480×38 SPINE-AND-LOOP** map: a readable surface SPINE with the spawn HUB at center (tile 240), difficulty ramping OUTWARD to the far edges, distinct zone SHAPES (PEAK vertical ladder · CANOPY climb · MEADOW rest+alcoves · EASTRUN pit-gauntlet · SUMMIT ascending staircase), a **parallel SKY HIGHWAY** (platforms link end-to-end so you can traverse at altitude OR on the ground), and an underground **CAVE NETWORK** carved into solid rock: three distinct systems (WEST/VIOLET shallow — antechamber→skinny tunnel→boss hall + single-entrance secret pocket · CENTRAL secret chamber · EAST/INDIGO deep two-tier — upper chamber→drop-shaft→deep halls joined by a crawl-tunnel + side pocket), 6 underground chests, foes in every chamber, cave voids you platform across to escape. Platform placement is enforced by **tools/map-geometry.mjs** (stable-math build gate: ≥2t headroom, no terrain burial, ≥3t vertical spacing). B38 fixed iPhone landscape/fullscreen: the canvas is sized from its own laid-out box (position:fixed;inset:0), not visualViewport, curing the "fits half the screen" bug. Since B28: same feature set; all six enemies are **grounded walkers**; the skill tree is **gone** (abilities always-on). For the authoritative running state see the **Definitive State** knowledge entry.
...
- **7 zones** (`ZB`) — surface: PEAK (BLUE), CANOPY (YELLOW), MEADOW (RED), EAST RUN (ORANGE, centered spawn), SUMMIT (GREEN); underground: DEPTHS (VIOLET), CAVERN (INDIGO). One contiguous **480×30** world (compact 3-band rebuild: SKY rows 1–17 · uniform 6-tile GROUND band rows 18–23 · CAVES rows 24–27 with single-level chamber floor row 28), each zone its own 5-color palette.

...
**State snapshot (2026-09-12):** B37 is **shipped & aligned** — git `main == origin == 3f58b9c`, Wavedash playtest build **mn77zd3d8pmjbe1nzzg8z9kqdh8e9fmz**, built at **12,809 / 13,312 B (503 B free, 96.2%)** (B37 = mobile: portrait shows a rotate-to-landscape prompt), save **v44**, single slot `uni_s0`. The world is a compact **480×30** 3-band map (SKY / uniform GROUND band / CAVES): **shallow single-level caves** (floor row 28), a **full-width developed underground** (39%→99% built), spawn **centered at tile 240**, and dense sky+cave platforming with chests/enemies/bosses spread evenly. Since B28: same feature set, byte-positive; all six enemies are **grounded walkers**; the skill tree is **gone** (abilities always-on). For the authoritative running state see the **Definitive State** knowledge entry.
